...
function NavBar({ onLogin }) {
  ...
  const handleMenuOptions = (op) => {
    ...
    if (op === 'opCreateProduct') {
      navigate('/create-product');
    }
    if (op === 'opUpdateProduct') {
      navigate('/update-product'); // Redireciona para a página de atualização de produto
    }
    if (op === 'opLogin') {
      navigate('/login');
    }
  ...
  if (userLogin) {
    pages = [
      ...
      {
        op: 'opUpdateProduct',
        text: 'Atualizar Produto' // Adiciona a opção para atualizar produtos
      }
    ];
